// Run tests:
// npx hardhat test test/partners/zksoul/zksoul.minter.test.js 
...
function calculateGasCosts(testName, receipt) {
  ...
  const gasCostEthereum = ethers.utils.formatUnits(String(Number(ethers.utils.parseUnits("40", "gwei")) * Number(receipt.gasUsed)), "ether");
  const gasCostZksync = ethers.utils.formatUnits(String(Number(ethers.utils.parseUnits("0.25", "gwei")) * Number(receipt.gasUsed)), "ether");

  console.log(testName + " gas cost (Ethereum): $" + String(Number(gasCostEthereum)*eth));
  console.log(testName + " gas cost (Zksync): $" + String(Number(gasCostZksync)*eth));
}

describe("zkSoul minter contract", function () {
  let tldContract;
  const tldName = ".zksoul";
  const tldSymbol = ".ZKSOUL";
  const tldPrice = 0;
  const tldRoyalty = 0;
  ...
  beforeEach(async function () {
    [signer, user1, user2, owner] = await ethers.getSigners();
