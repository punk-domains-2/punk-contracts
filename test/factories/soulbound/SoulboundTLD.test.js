// npx hardhat test test/factories/soulbound/SoulboundTLD.test.js
...
describe("SoulboundPunkTLD", function () {
  let contract;
  let factoryContract;
  ...
  const domainName = ".soulbound";
  const domainSymbol = ".SOULBOUND";
  ...
  beforeEach(async function () {
    ...
    const PunkTLD = await ethers.getContractFactory("SoulboundPunkTLD");
  ...
  it("should fail to transfer domain to another user", async function () {
    ...
    // try to transfer domain from signer to another user
    // will fail because it's a soulbound domain (non-transferable domain)
    ...
    expect(defaultNameAfterSigner).to.equal(newDomainName); // old owner still keeps this domain

    const defaultNameAfterAnother = await contract.defaultNames(anotherUser.address);
    expect(defaultNameAfterAnother).to.be.empty; // another user does not get the domain

    // get owner
    const domainOwnerAfter = await contract.ownerOf(tokenId);
    expect(domainOwnerAfter).to.equal(signer.address);

    // get domain data by domain name
    const firstDomainDataAfter = await contract.domains(newDomainName);
    expect(firstDomainDataAfter.name).to.equal(newDomainName);
    expect(firstDomainDataAfter.holder).to.equal(signer.address);
  });
