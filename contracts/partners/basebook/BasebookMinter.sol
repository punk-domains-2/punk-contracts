...
// Minter contract for Basebook ID
contract BasebookMinter is Ownable, ReentrancyGuard {
  ...
  mapping (address => mapping (uint256 => bool)) public alreadyMinted; // nftAddress => tokenId => bool, whitelisted NFTs that have already minted a domain
  ...
  function canMintFree(address user_) public view returns(bool) {
    for (uint256 i = 0; i < whitelistedNfts.length; i++) {
      address nftAddress = whitelistedNfts[i];
      uint256 balance = IERC721(nftAddress).balanceOf(user_);

      if (balance > 0) {
        for (uint256 j = 0; j < balance; j++) {
          uint256 tokenId = IERC721Enumerable(nftAddress).tokenOfOwnerByIndex(user_, j);
          if (!alreadyMinted[nftAddress][tokenId]) {
            return true;
          }
        }
      }
  ...
  function mintFreeDomain(
    ...
    uint256 domainLength = strings.len(strings.toSlice(_domainName));
    require(domainLength >= 5, "Domain length must be 5 characters or more");
    
    for (uint256 i = 0; i < whitelistedNfts.length; i++) {
      address nftAddress = whitelistedNfts[i];
      uint256 balance = IERC721(nftAddress).balanceOf(msg.sender);

      if (balance > 0) {
        for (uint256 j = 0; j < balance; j++) {
          uint256 tokenId = IERC721Enumerable(nftAddress).tokenOfOwnerByIndex(msg.sender, j);
          if (!alreadyMinted[nftAddress][tokenId]) {
            alreadyMinted[nftAddress][tokenId] = true;
    ...
    revert("No free domains available");
  }

  // OWNER

  /// @notice This adds a whitelisted NFT
  ...
  /// @notice Remove a whitelisted NFT
  ...
  /// @notice This changes paused state in the minter contract
